...
//                $this->session->set_userdata('current_language' , $current_language);
...
//        echo $email;
        $query_admin = $this->db->get_where('admin' , array(	'email' => $email ));
...
//            $data['class_id']    = $this->input->post('class_id');
    ...
    function get_student_list($class_id='',$group_id='',$sec_id='')
    {
        ...
        $page_data['students'] = $this->db->get_where('student' , array('class_id'=>$class_id,'dep_id'=>$group_id,"sec_id"=>$sec_id))->result_array();
         ...
         $this->load->view('backend/admin/get_student_list', $page_data);
//        echo "yes";
    }
...
//            $data['teacher_id']   = $this->input->post('teacher_id');
    ...
    function section($param1 = '',$param2='',$param3='')
    {
        ...
        if($param1=='add')
        {
            ...
            $this->session->set_flashdata('class_type', 'alert alert-info');
            $this->session->set_flashdata('flash_message', get_phrase('successfully_added'));
            redirect(base_url() . 'index.php?admin/add_section/' . $data['class_id'], 'refresh');
        }
        if($param1 == 'edit')
        {
            $class_id=$param3;
            ...
            $this->session->set_flashdata('class_type', 'alert alert-info');
            $this->session->set_flashdata('flash_message', get_phrase('updated'));
            redirect(base_url() . 'index.php?admin/section_information/' . $class_id, 'refresh');
        }
        if ($param1 == 'delete') {
            $class_id=$param3;
            $this->db->where('sec_id', $param2);
    ...
    function get_sec_name($param1='',$param2='')
    {
        ...
        $data['dep_id']=$param2;
        $data1=  $this->load->view("backend/admin/get_sec_name",$data);
        echo $data1;
    }
...
//        echo $data['class_id'].' '.$data['dep_id'].' '.$data['sec_id'];
        $data1=  $this->load->view("backend/admin/get_sub_name",$data);
//        echo $data['class_id'];
    }
    ...
    function marks($exam_id = '', $class_id = '', $dep_id = '', $sec_id = '', $subject_id = '')
    {
        ...
        if ($this->input->post('operation') == 'selection') {
            ...
            if ($page_data['exam_id'] > 0 && $page_data['class_id'] > 0 && $page_data['subject_id'] > 0) {
                redirect(base_url() . 'index.php?admin/marks/' . $page_data['exam_id'] . '/' . $page_data['class_id'] . '/' . $page_data['dep_id'] . '/' . $page_data['sec_id'] . '/' . $page_data['subject_id'], 'refresh');
            } else {
...
//                        $attendance_status  =   $this->input->post('status_' . $row['student_id']);
            ...
            $this->session->set_flashdata('flash_message', get_phrase('updated'));
			 ...
			 redirect(base_url() . 'index.php?admin/marks/' . $this->input->post('exam_id') . '/'. $this->input->post('class_id') . '/'. $this->input->post('dep_id') . '/' . $this->input->post('sec_id') . '/' . $this->input->post('subject_id'), 'refresh');
    ...
    function class_routine($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            $this->session->set_flashdata('class_type', 'alert alert-info');
            $this->session->set_flashdata('flash_message', get_phrase('successfully_added'));
        redirect(base_url() . 'index.php?admin/add_cl', 'refresh');
...
//        $page_data['page_title'] = get_phrase('manage_class_routine');
	...
	function manage_attendance($class_id='',$group_id='',$sec_id)
	{
                ...
                if($_POST['save_attendence'])
		{
			...
			//redirect(base_url() . 'index.php?admin/manage_attendance/'.$date.'/'.$month.'/'.$year.'/'.$class_id .'/'.$group_id.'/'.$sec_id, 'refresh');
	...
	{
		redirect(base_url() . 'index.php?admin/manage_attendance/'.$this->input->post('class_id').'/'.$this->input->post('dep_id').'/'.$this->input->post('sec_id') , 'refresh');
	}
...
//          $file_name="adfaf";
          //echo 'uploads/doc/'.$acc_type.'/'.$file_name.'';
            ...
            redirect(base_url() . 'index.php?admin/navigation/modal_document_upload/'.$data['user_id'].'/'.$data['role'], 'refresh');
         }
         if ($action == 'delete') {
            ...
            // echo $doc_id.' '.$user_id.' '.$role;
             ...
             redirect(base_url() . 'index.php?admin/navigation/modal_document_upload/'.$user_id.'/'.$role, 'refresh');
         }
